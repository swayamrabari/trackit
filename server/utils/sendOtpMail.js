...
function buildOtpEmailHtml({ toName, code }) {
  ...
  return `
    <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 480px; margin: 0 auto; padding: 32px 24px; background: #fafafa; border-radius: 8px;">
      <h2 style="color: #333; margin: 0 0 16px;">Your OTP Code</h2>
      <p style="color: #555; font-size: 15px;">Hey <strong>${safeName}</strong>,</p>
      <p style="color: #555; font-size: 15px;">Use the code below to verify:</p>
...
const sendOtpMail = async (toEmail, otp) => {
  try {
    ...
    await sendThroughBrevoApi({
      contextLabel: 'OTP email',
      payload: {
        sender: getFromField(),
        to: [{ email: recipient }],
        subject: 'Your OTP Code',
        htmlContent: buildOtpEmailHtml({ toName, code }),
        textContent: `Your OTP code is ${code}. It is valid for 10 minutes.`,
      },
    });

    logger.info('OTP email sent successfully via Brevo', { toEmail });
  } catch (error) {
    logger.error('Brevo email error', { message: error.message, toEmail });
